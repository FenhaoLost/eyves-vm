...
| 6 | ~~**OpenAPI 缺 `operationId`**：43/43 个操作缺失~~ | [openapi.yaml](file:///workspace/eyves-vm/api/openapi.yaml)（全文件） | ~~P1~~ | ✅ **已修复**：43/43 全部补齐，命名约定 `资源.动作` 全小写；实测 `missing operationId: 0`、`duplicate: 0`。**附带修复**：发现并修复 `CreateInstanceRequest.days` / `RenewRequest.days` 的价格漏洞（客户端可传 1..1095 天而价格是套餐固定值），改为必填 `cycle` + `prices[]`，见 [03-api.md §3.14 A11](file:///workspace/eyves-vm/docs/03-api.md) |
